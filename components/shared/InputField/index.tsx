...
const InputField = (props: InputFieldProps) => {
  const { label, message, required, disabled, size, right, className, inputProps, labelProps } = props

  return(
    <div className={cn(inputFieldVariant({ className, right}))}>
      <Label required={required} sizes={size} { ...labelProps } >{ label }</Label>
      <Input disabled={disabled} required={required} sizes={size} { ...inputProps } theme={message?.type} />
      {message && (
        <span className={cn(messageInputFieldVariant({ variant: message.type, sizes: size }))}>
          {message.text}
        </span>
      )}
    </div>
