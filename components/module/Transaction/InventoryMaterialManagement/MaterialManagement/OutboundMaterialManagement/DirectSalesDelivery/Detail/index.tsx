'use client';

import React from 'react';
import {
  Drawer,
  DrawerBody,
  DrawerContent,
  DrawerEndHeader,
  DrawerFooter,
  DrawerHeader,
} from '@components/ui/Drawer';
import { useDrawerStore } from '@stores/useDrawerStore';
import { useSetValueForm } from '@hooks/useSetValueForm';
import { useForm } from 'react-hook-form';
import { IconHistory, IconPencil } from '@tabler/icons-react';
import { Button } from '@components/ui/Button';
import DirectSalesDeliveryHeaderForm from '../Form/HeaderForm';
import DirectSalesDeliveryDetailForm from '../Form/DetailForm';

const DetailDirectSalesDelivery = () => {
  const {
    isOpenDetail,
    closeDetailDrawer,
    openEditDrawer,
    openHistoryLogDrawer,
  } = useDrawerStore();
  const detail_data = useDrawerStore(
    (state) => state.detail_data
  ) as DirectSalesDeliveryFormBody;

  const { setValue, watch, register } = useForm<DirectSalesDeliveryFormBody>();

  useSetValueForm<DirectSalesDeliveryFormBody>(detail_data, setValue);

  return (
    <Drawer onClose={closeDetailDrawer} open={isOpenDetail}>
      <DrawerContent>
        <DrawerHeader>
          <DrawerEndHeader>
            <Button
              variant="backDrawer"
              icon={{ size: 'large', icon: IconPencil, color: 'White' }}
              type="submit"
              onClick={openEditDrawer}
            >
              Edit
            </Button>
          </DrawerEndHeader>
        </DrawerHeader>
        <form>
          <DrawerBody>
            <DirectSalesDeliveryHeaderForm
              watch={watch}
              register={register}
              disableAll
            />
            <DirectSalesDeliveryDetailForm
              watch={watch}
              register={register}
              formType="detail"
              disableAll
            />
          </DrawerBody>
        </form>
        <DrawerFooter>
          <Button
            variant="backDrawer"
            className="w-7"
            size="icon"
            icon={{ size: 'large', icon: IconHistory, color: 'dark' }}
            type="submit"
            onClick={() => {
              openHistoryLogDrawer({
                code: detail_data?.document_number || '',
                category: 'DirectSalesDelivery',
              });
            }}
          />
        </DrawerFooter>
      </DrawerContent>
    </Drawer>
  );
};

export default DetailDirectSalesDelivery;
